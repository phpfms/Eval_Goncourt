...
Cette classe gère les relations entre les identités et les entités.
"""
...
class IdentityEntityDao(Dao[IdentityEntity]):
    ...
    def read(self, *id_entity: int) -> Optional[IdentityEntity]:
        ...
        try:
            ...
            print(f"Erreur lors de la lecture de la relation Identity / Entity : {error}")
            return None
    ...
    def update(self, identity_entity: IdentityEntity) -> bool:
        ...
        try:
            ...
            Dao.connection.rollback()
            print(f"Erreur lors de la modification de la relation Identity / Entity : {error}")
            return False
    ...
    def delete_by_entity(self, id_entity: int) -> bool:
        """Supprime toutes les relations Identity / Entity liées à une entité."""
    ...
    def delete_roles_by_identity(self, id_identity: int) -> bool:
        ...
        try:
            ...
            Dao.connection.rollback()
            print(f"Erreur lors de la suppression des rôles directs : {error}")
            return False

    def find_identities_by_entity(self, id_entity: int) -> list[tuple[int, int]]:
        # Règle métier : seules les relations réellement rattachées au livre
        # sont recherchées. Une relation doit contenir une identité et un rôle.
        ...
        try:
            with Dao.connection.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT DISTINCT fk_id_identity, fk_id_role
                    FROM identity_entity
                    WHERE fk_id_entity = %s
                      AND fk_id_identity IS NOT NULL
                      AND fk_id_role IS NOT NULL
            ...
            # Choix technique : le DAO retourne uniquement les identifiants.
            # Les objets Identity et Role seront construits par leurs Business
            # respectifs afin de conserver la séparation DAO / Business / Display.
            relations = []
            for record in records:
                relations.append((record["fk_id_identity"], record["fk_id_role"]))

            return relations
        except Exception as error:
            print(f"Erreur lors de la recherche des personnes liées au livre : {error}")
            return []